...
void pid() {
  ...
  S1 = (1-alpha)*S1+(alpha*distances[0]); // front 
  S2 = (1-alpha)*S2+(alpha*distances[1]); // Left 
  S3 = (1-alpha)*S3+(alpha*distances[2]); // back
  S4 = (1-alpha)*S4+(alpha*distances[3]); // right
  //  Serial.printf("S1: %.2f    S2: %.2f    S3: %.2f    S4: %.2f\n", S1, S2, S3, S4);
  ...
  Y = -S3;
  // if(S3>150 && S1<150) {
  //   Serial.println("SENSOR CHANGED");
  //   Y = -S1;
  //   target_y = 320 - target_y;
  // }

  // float Yb = S3 + BOT_L * 0.5;                    // from back wall
  // float Yf = ARENA_H - (S1 + BOT_L * 0.5);        // from back wall

  // float wb = 1.0 / (S3 + 1.0);   // back sensor weight
  // float wf = 1.0 / (S1 + 1.0);   // front sensor weight
  ...
  // float Y_abs = wb * Yb + wf * Yf;
  // float Y = Y_abs - (ARENA_H * 0.5);
 ...
 Serial.printf("X: %.2f    y: %.2f\n", X, Y);
  ...
  // bot_y = X * cos(bot_theta) - Y * sin(bot_theta);
  // bot_x = X * sin(bot_theta) + Y * cos(bot_theta);
...
//  bot_y = constrain(bot_y, 0, ARENA_H);
  ...
  // if ( S4 <= 0 || S5 <= 0) {
    ...
    // ===== GO TO TARGET =====
    float ex = target_x - bot_x; //-40  0
    float ey = target_y - bot_y; //0    -100   -150
  // x = -y; y = x  -> old
  // x = x; y = -y
    // Convert world → robot frame
    float vx =  cos(bot_theta)*ex + sin(bot_theta)*ey; //    -100  -150
    float vy = -sin(bot_theta)*ex + cos(bot_theta)*ey; //40   0

    // float vy =  cos(bot_theta)*ex + sin(bot_theta)*ey; //    -100  -150
    // float vx = -sin(bot_theta)*ex + cos(bot_theta)*ey; //40   0
...
void input() {
  ...
  if(S3>150 && S1<150) {
    Serial.println("SENSOR CHANGED");
    Y = -S1;
    target_y = -1*(260+input.substring(4).toFloat());
  }
  else{
