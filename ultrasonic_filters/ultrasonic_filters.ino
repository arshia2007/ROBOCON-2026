...
// ultrasonic filter
float alpha = 0.3;   // 0.1 = more smooth, 0.5 = more responsive
float filteredDist = 0;

float EMA(float newVal) {
  filteredDist = alpha * newVal + (1 - alpha) * filteredDist;
  return filteredDist;
}

float lastDist = 0;
float MAX_JUMP = 30; // cm

float rejectOutliers(float val) {
  if (abs(val - lastDist) > MAX_JUMP)
    return lastDist;
  lastDist = val;
  return val;
}
...
void pid() {
  ...
  // apply filter
  S2 = rejectOutliers(S2);
  S2 = EMA(S2);

  S3 = rejectOutliers(S3);
  S3 = EMA(S3);
